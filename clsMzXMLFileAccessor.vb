...
#Region "Classwide Variables"
	...
	Protected mMSRunRegEx As Text.RegularExpressions.Regex
	Protected mScanStartElementRegEx As Text.RegularExpressions.Regex
	Protected mPeaksEndElementRegEx As Text.RegularExpressions.Regex

	Protected mScanCountRegEx As Text.RegularExpressions.Regex
	Protected mScanNumberRegEx As Text.RegularExpressions.Regex
...
#End Region
	...
	Public Overrides Function GetSourceXMLHeader(ByVal intScanCountTotal As Integer, ByVal sngStartTimeMinutesAllScans As Single, ByVal sngEndTimeMinutesAllScans As Single) As String
		...
		Dim reStartTime As Text.RegularExpressions.Regex
		Dim reEndTime As Text.RegularExpressions.Regex
		Dim objMatch As Text.RegularExpressions.Match
		...
		Try
			...
			If GetSpectrumReadyStatus(True) Then
				...
				ElseIf intSpectrumIndex >= 0 And intSpectrumIndex < mIndexedSpectrumInfoCount Then
					...
					If blnSuccess Then
						If DEBUG_MODE Then
							srOutfile = New IO.StreamWriter(System.IO.Path.GetFileNameWithoutExtension(mInputFilePath) & "_Index" & intSpectrumIndex.ToString & "_excerpt.xml", False)
	...
	Protected Overrides Function LoadExistingIndex() As Boolean
		...
		Dim objStringBuilder As Text.StringBuilder
		...
		Try
			...
			Do While mBinaryTextReader.ReadLine(clsBinaryTextReader.ReadDirectionConstants.Reverse)
				...
				If blnExtractTextToEOF Then
					objStringBuilder = New Text.StringBuilder
					...
					If blnIndexLoaded Then
						...
						If mIndexedSpectrumInfoCount > 0 Then
							...
							mErrorMessage = "Index embedded in the input file (" & IO.Path.GetFileName(mInputFilePath) & ") is corrupt: first byte offset (" & mIndexedSpectrumInfo(0).ByteOffsetStart.ToString & ") does not point to a " & SCAN_START_ELEMENT & " element"
	...
	Protected Function ParseMzXMLOffsetIndex(ByRef strTextStream As String) As Boolean
		...
		Try
			...
			objXMLReader = New Xml.XmlTextReader(New IO.StringReader(strTextStream))
	...
	Public Overrides Function ReadAndCacheEntireFile() As Boolean
		...
		Try
			...
			Else
				...
				MyBase.ResetProgress("Indexing " & IO.Path.GetFileName(mInputFilePath))
