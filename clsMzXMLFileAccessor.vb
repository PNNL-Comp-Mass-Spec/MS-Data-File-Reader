...
#Region "Classwide Variables"
    Protected mXmlFileReader As clsMzXMLFileReader

    Protected mCurrentSpectrumInfo As clsSpectrumInfoMzXML

    Protected mXmlFileHeader As String
    Protected mAddNewLinesToHeader As Boolean

    Protected mMSRunFound As Boolean
    Protected mIgnoreEmbeddedIndex As Boolean

    Protected mMSRunRegEx As System.Text.RegularExpressions.Regex
    Protected mScanStartElementRegEx As System.Text.RegularExpressions.Regex
    Protected mPeaksEndElementRegEx As System.Text.RegularExpressions.Regex

    Protected mScanCountRegEx As System.Text.RegularExpressions.Regex
    Protected mScanNumberRegEx As System.Text.RegularExpressions.Regex
...
#End Region
    ...
    Protected Overrides Function AdvanceFileReaders(ByVal eElementMatchMode As clsMSDataFileAccessorBaseClass.emmElementMatchModeConstants) As Boolean
                    ...
                    If objMatch.Success Then
                        ...
                        If eElementMatchMode = emmElementMatchModeConstants.StartElement Then
                            ...
                            Else
                                ...
                                If strInFileCurrentLineSubstring.IndexOf(PEAKS_END_ELEMENT) < 0 Then
                                    blnMatchFound = False
    ...
    Protected Function ExtractIndexOffsetFromTextStream(ByRef strTextStream As String) As Long
        ...
        Try
            ...
            intMatchIndex = strTextStream.IndexOf(INDEX_OFFSET_START_ELEMENT)
    ...
    Protected Overrides Function ExtractTextBetweenOffsets(ByVal strFilePath As String, ByVal lngStartByteOffset As Long, ByVal lngEndByteOffset As Long) As String
        ...
        If Not strExtractedText Is Nothing AndAlso strExtractedText.Length > 0 Then
            ...
            intMatchIndex = strExtractedText.IndexOf(PEAKS_END_ELEMENT)
            If intMatchIndex >= 0 Then
                intMatchIndex = strExtractedText.IndexOf(SCAN_END_ELEMENT, intMatchIndex)
                If intMatchIndex >= 0 Then
    ...
    Public Overrides Function GetSourceXMLHeader(ByVal intScanCountTotal As Integer, ByVal sngStartTimeMinutesAllScans As Single, ByVal sngEndTimeMinutesAllScans As Single) As String
        Dim strHeaderText As String
        Dim intAsciiValue As Integer

        Dim reStartTime As System.Text.RegularExpressions.Regex
        Dim reEndTime As System.Text.RegularExpressions.Regex
        Dim objMatch As System.Text.RegularExpressions.Match
        ...
        Try
            ...
            strStartTimeSOAP = clsMSXMLFileReaderBaseClass.ConvertTimeFromTimespanToXmlDuration(New TimeSpan(CLng(sngStartTimeMinutesAllScans * System.TimeSpan.TicksPerMinute)), True)
            strEndTimeSOAP = clsMSXMLFileReaderBaseClass.ConvertTimeFromTimespanToXmlDuration(New TimeSpan(CLng(sngEndTimeMinutesAllScans * System.TimeSpan.TicksPerMinute)), True)
        ...
        Try
            ...
            If GetSpectrumReadyStatus(True) Then
                ...
                ElseIf intSpectrumIndex >= 0 And intSpectrumIndex < mIndexedSpectrumInfoCount Then
                    ...
                    blnSuccess = ExtractTextFromFile(mInputFilePath, mIndexedSpectrumInfo(intSpectrumIndex).ByteOffsetStart, mIndexedSpectrumInfo(intSpectrumIndex).ByteOffsetEnd, strExtractedText, 1, 0, 0)

                    If blnSuccess Then
                        If DEBUG_MODE Then
                            srOutfile = New System.IO.StreamWriter(System.IO.Path.GetFileNameWithoutExtension(mInputFilePath) & "_Index" & intSpectrumIndex.ToString & "_excerpt.xml", False)
                            srOutfile.WriteLine(strExtractedText)
                            srOutfile.Close()
                            srOutfile = Nothing
                        End If

                        mXmlFileReader.SkipBinaryData = blnHeaderInfoOnly
                        If mXmlFileReader.OpenTextStream(strExtractedText) Then
                    ...
                    Else
                        If mXmlFileReader.ErrorMessage.Length > 0 Then
                            mErrorMessage = mXmlFileReader.ErrorMessage
                        Else
                            mErrorMessage = "Unknown error extracting and parsing spectrum from mzData file"
                        End If
                        blnSuccess = False
                    End If
                Else
    ...
    Protected Sub InitializeObjectVariables()
    ...
    Protected Overrides Function LoadExistingIndex() As Boolean
        ...
        Try
            ...
            Do While mBinaryTextReader.ReadLine(clsBinaryTextReader.ReadDirectionConstants.Reverse)
                ...
                intCharIndex = strCurrentLine.IndexOf(INDEX_OFFSET_START_ELEMENT)
                If intCharIndex >= 0 Then
                    ...
                    intCharIndexEnd = strCurrentLine.IndexOf(INDEX_OFFSET_END_ELEMENT, intCharIndex + INDEX_OFFSET_START_ELEMENT.Length)
                    ...
                    If intCharIndexEnd > 0 Then
                        ...
                        If lngIndexOffset > 0 Then
                            ...
                            If strCurrentLine.IndexOf(INDEX_START_ELEMENT) >= 0 Then
                ...
                If Not blnExtractTextToEOF Then
                    intCharIndex = strCurrentLine.IndexOf(MSRUN_END_ELEMENT)
                    If intCharIndex >= 0 Then
                        ' </msRun> element found
                ...
                If blnExtractTextToEOF Then
                    ...
                    If blnIndexLoaded Then
                        ...
                        If mIndexedSpectrumInfoCount > 0 Then
                            ...
                            mErrorMessage = "Index embedded in the input file (" & System.IO.Path.GetFileName(mInputFilePath) & ") is corrupt: first byte offset (" & mIndexedSpectrumInfo(0).ByteOffsetStart.ToString & ") does not point to a " & SCAN_START_ELEMENT & " element"
                    ...
                    If blnIndexLoaded Then
                        ...
                        Do While mBinaryTextReader.ReadLine(clsBinaryTextReader.ReadDirectionConstants.Forward)
                            ...
                            intCharIndex = strCurrentLine.IndexOf(SCAN_START_ELEMENT)
                            If intCharIndex >= 0 Then
    ...
    Protected Function ParseMzXMLOffsetIndex(ByRef strTextStream As String) As Boolean
        ...
        Dim intCurrentScanNumber As Integer
        Dim intPreviousScanNumber As Integer

        Dim lngCurrentScanByteOffsetStart As Long
        Dim lngPreviousScanByteOffsetStart As Long

        Dim blnIndexLoaded As Boolean
        Dim blnParseIndexValues As Boolean

        Try
            blnIndexLoaded = False
            blnParseIndexValues = True

            intCurrentScanNumber = -1
            intPreviousScanNumber = -1

            lngCurrentScanByteOffsetStart = -1
            lngPreviousScanByteOffsetStart = -1

            strCurrentElement = String.Empty

            objXMLReader = New System.Xml.XmlTextReader(New System.IO.StringReader(strTextStream))

            ' Skip all whitespace
            objXMLReader.WhitespaceHandling = Xml.WhitespaceHandling.None

            blnReadSuccessful = True
            Do While blnReadSuccessful AndAlso _
                (objXMLReader.ReadState = Xml.ReadState.Initial Or _
                objXMLReader.ReadState = Xml.ReadState.Interactive)

                blnReadSuccessful = objXMLReader.Read()

                If blnReadSuccessful AndAlso objXMLReader.ReadState = Xml.ReadState.Interactive Then
                    ...
                    ElseIf objXMLReader.NodeType = Xml.XmlNodeType.Text Then
                        If blnParseIndexValues AndAlso strCurrentElement = OFFSET_ELEMENT_NAME Then
                            If Not objXMLReader.NodeType = Xml.XmlNodeType.Whitespace And objXMLReader.HasValue Then
                                Try
    ...
    Public Overrides Function ReadAndCacheEntireFile() As Boolean
        ...
        Try
            ...
            Else
                ...
                MyBase.ResetProgress("Indexing " & System.IO.Path.GetFileName(mInputFilePath))
    ...
    Protected Function ReadMZXmlFile() As Boolean
        ...
        Try
            ...
            Do
                ...
                blnSpectrumFound = AdvanceFileReaders(clsMSDataFileAccessorBaseClass.emmElementMatchModeConstants.StartElement)
                If blnSpectrumFound Then
                    ...
                    blnSpectrumFound = AdvanceFileReaders(clsMSDataFileAccessorBaseClass.emmElementMatchModeConstants.EndElement)
                    If blnSpectrumFound Then
                        If mCharSize > 1 Then
    ...
    Protected Sub StoreFinalIndexEntry(ByVal intScanNumber As Integer, ByVal lngByteOffsetStart As Long)
    ...
    Protected Sub UpdateXmlFileHeaderScanCount(ByRef strHeaderText As String)
        UpdateXmlFileHeaderScanCount(strHeaderText, 1)
    End Sub

    Protected Sub UpdateXmlFileHeaderScanCount(ByRef strHeaderText As String, ByVal intScanCountTotal As Integer)
