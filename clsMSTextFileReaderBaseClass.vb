...
#Region "Classwide Variables"
	...
	Protected srInFile As IO.TextReader
...
#End Region
	...
	Public Function ExtractScanInfoFromDtaHeader(ByVal strSpectrumHeader As String, <Out()> ByRef intScanNumberStart As Integer, <Out()> ByRef intScanNumberEnd As Integer, <Out()> ByRef intScanCount As Integer, <Out()> ByRef intCharge As Integer) As Boolean
		...
		Static reDtaHeaderScanAndCharge As Text.RegularExpressions.Regex = New Text.RegularExpressions.Regex(".+\.(\d+)\.(\d+)\.(\d*)$", Text.RegularExpressions.RegexOptions.Compiled)
	...
	Public Overrides Function OpenFile(ByVal strInputFilePath As String) As Boolean
		...
		Try
			...
			objStreamReader = New IO.StreamReader(New IO.FileStream(strInputFilePath, IO.FileMode.Open, IO.FileAccess.Read, IO.FileShare.Read))
	...
	Public Overrides Function OpenTextStream(ByRef strTextStream As String) As Boolean
		...
		Try
			...
			srInFile = New IO.StringReader(strTextStream)
