...
#Region "Classwide Variables"
    ...
    Protected mBinaryReader As System.IO.FileStream
...
#Region "Processing Options and Interface Functions"
    ...
    Public Property FileSystemMode() As FileSystemModeConstants
        ...
        End Get
        Set(ByVal Value As FileSystemModeConstants)
    ...
    Public Property InputFileEncoding() As InputFileEncodingConstants
        ...
        End Get
        Set(ByVal Value As InputFileEncodingConstants)
            SetInputFileEncoding(Value)
        End Set
    End Property
#End Region

    Public Function ByteAtBOF(ByVal lngBytePosition As Long) As Boolean
    ...
    Public Function ByteAtEOF(ByVal lngBytePosition As Long) As Boolean
    ...
    Protected Sub LogErrors(ByVal strCallingFunction As String, ByVal strErrorDescription As String)
    ...
    Public Sub MoveToByteOffset(ByVal lngByteOffset As Long)
        ...
        Try
            If Not mBinaryReader Is Nothing AndAlso mBinaryReader.CanRead Then
                ...
                If lngByteOffset < mByteBufferFileOffsetStart Then
                    ...
                    mBinaryReader.Seek(mByteBufferFileOffsetStart, IO.SeekOrigin.Begin)
    ...
    Public Function OpenFile(ByVal InputFilePath As String) As Boolean
        Return OpenFile(InputFilePath, IO.FileShare.Read)
    End Function

    Public Function OpenFile(ByVal InputFilePath As String, ByVal share As System.IO.FileShare) As Boolean
    ...
    Public Function ReadLine(ByVal eDirection As ReadDirectionConstants) As Boolean
            ...
            If Not mBinaryReader Is Nothing AndAlso mBinaryReader.CanRead Then
                ...
                Do While Not blnMatchFound
                    ...
                    If blnTerminatorFound Then
                        ...
                        Select Case mInputFileEncoding
                            Case InputFileEncodingConstants.Ascii, InputFileEncodingConstants.UTF8
                                ...
                                If mLineTerminator1Code <> 0 AndAlso _
                                   ...
                                   mByteBuffer(intMatchingTextIndexEnd - mCharSize) = mLineTerminator1Code Then
                                    ...
                                    mCurrentLineTerminator = System.Convert.ToChar(mByteBuffer(intMatchingTextIndexEnd - mCharSize)) & System.Convert.ToChar(mByteBuffer(intMatchingTextIndexEnd))
                                ElseIf mByteBuffer(intMatchingTextIndexEnd) = mLineTerminator2Code Then
                                    intLineTerminatorLength = 1
                                    mCurrentLineTerminator = System.Convert.ToChar(mByteBuffer(intMatchingTextIndexEnd))
                                Else
                                ...
                                Else
                                    If mInputFileEncoding = InputFileEncodingConstants.UTF8 Then
                                        ...
                                        mCurrentLineText = System.Convert.ToString(System.Text.Encoding.UTF8.GetChars(mByteBuffer, intMatchingTextIndexStart, intBytesToRead))
                                    Else
                                        ' Extract the data between intMatchingTextIndexStart and intMatchingTextIndexEnd, excluding any line terminator characters
                                        mCurrentLineText = System.Convert.ToString(System.Text.Encoding.ASCII.GetChars(mByteBuffer, intMatchingTextIndexStart, intBytesToRead))
                                    End If
                                End If
                            ...
                            Case InputFileEncodingConstants.UnicodeNormal
                                ...
                                If intBytesToRead <= 0 Then
                                    ...
                                    mCurrentLineText = String.Empty
                                Else
                                    mCurrentLineText = System.Convert.ToString(System.Text.Encoding.Unicode.GetChars(mByteBuffer, intMatchingTextIndexStart, intBytesToRead))
                                End If
                            ...
                            Case InputFileEncodingConstants.UnicodeBigEndian
                                ...
                                If mLineTerminator1Code <> 0 AndAlso _
                                   ...
                                   mByteBuffer(intMatchingTextIndexEnd - mCharSize + 1) = mLineTerminator1Code Then
                                    intLineTerminatorLength = 2
                                    mCurrentLineTerminator = System.Convert.ToChar(mByteBuffer(intMatchingTextIndexEnd - mCharSize + 1)) & System.Convert.ToChar(mByteBuffer(intMatchingTextIndexEnd + 1))
                                ElseIf mByteBuffer(intMatchingTextIndexEnd) = 0 AndAlso _
                                       intMatchingTextIndexEnd + 1 < mByteBufferCount AndAlso _
                                       mByteBuffer(intMatchingTextIndexEnd + 1) = mLineTerminator2Code Then
                                    intLineTerminatorLength = 1
                                    mCurrentLineTerminator = System.Convert.ToChar(mByteBuffer(intMatchingTextIndexEnd + 1))
                                Else
                                    ' No line terminator (this is probably the last line of the file or else the user called MoveToByteOffset with a location in the middle of a line)
                                    intLineTerminatorLength = 0
                                    mCurrentLineTerminator = String.Empty
                                ...
                                If intBytesToRead <= 0 Then
                                    ...
                                    mCurrentLineText = String.Empty
                                Else
                                    mCurrentLineText = System.Convert.ToString(System.Text.Encoding.BigEndianUnicode.GetChars(mByteBuffer, intMatchingTextIndexStart, intBytesToRead))
                                End If
                        ...
                        If eDirection <> mReadLineDirectionSaved AndAlso _
                           mCurrentLineByteOffsetStartSaved >= 0 AndAlso _
                           mCurrentLineByteOffsetStart = mCurrentLineByteOffsetStartSaved AndAlso _
                           Not mCurrentLineTextSaved Is Nothing AndAlso _
                        ...
                        If eDirection = ReadDirectionConstants.Forward Then
                            ...
                            If mByteBufferNextLineStartIndex > 0 Then
                                ...
                                If mByteBufferFileOffsetStart + mByteBufferCount <> mBinaryReader.Position Then
                                    ...
                                    mBinaryReader.Seek(mByteBufferFileOffsetStart + mByteBufferCount, IO.SeekOrigin.Begin)
                                End If
                            Else
                                intSearchIndexStartOffset = mByteBufferCount
    ...
    Protected Sub SetInputFileEncoding(ByVal EncodingMode As InputFileEncodingConstants)
